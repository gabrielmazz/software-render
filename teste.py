...
class Cube(object):
    ...
    def rotate(self, alpha=None, beta=None, gamma=None):
        ...
        if changed:
            ...
            self.points = map(self._rotate_point, points)
...
class MainWindow(Tk):
    def __init__(self):
        ...
        Scale(self, label='beta', from_=-180, to=180, resolution=1, orient=VERTICAL, command=self._set_beta).grid(row=0, column=0, sticky=N+S)
        Scale(self, label='alpha', from_=-180, to=180, resolution=1, orient=HORIZONTAL, command=self._set_alpha).grid(row=1, column=1, sticky=W+E)
        Scale(self, label='gamma', from_=-180, to=180, resolution=1, orient=HORIZONTAL, command=self._set_gamma).grid(row=2, column=1, sticky=W+E)
